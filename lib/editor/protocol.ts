import { z } from 'zod';
import { editValuesSchema, itemKeySchema, newNodeSchema, newRelationshipSchema, connectionValuesSchema, geometrySchema, pointSchema, viewSchema, LIMITS, type Locator } from '../graph/types';
...
export const editorRequestSchema = z.discriminatedUnion('op', [
  ...
  request('saveConnection', z.tuple([itemKeySchema, revision, connectionValuesSchema])),
  request('setPersonalEdit', z.tuple([itemKeySchema, revision, editValuesSchema])),
  request('removeItem', z.tuple([itemKeySchema, revision])),
  request('savePosition', z.tuple([itemKeySchema, geometrySchema])),
