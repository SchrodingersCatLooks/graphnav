...
import { GraphCanvas, type Selection } from '../../components/graph/GraphCanvas';
...
function Workspace() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const id = rows.find((graph) => graph.id === requested)?.id ?? rows[0]?.id;
        if (id) await load(id);
  ...
  return <main className="workspace">
    <header className="workspace-header">
      <a className="brand" href="workspace.html" onClick={(event) => { if (dirty || busy) event.preventDefault(); }}><GraphMark size={28} /><span>GraphNav</span></a>
      <span className="workspace-kicker">MY MAPS</span>
    ...
    <div className="workspace-body">
      <aside className="sidebar" aria-label="Map editor">
        <section className="map-picker">
          ...
          <form onSubmit={(event) => { event.preventDefault(); void perform(async () => { const graph = await repository.createGraph(newMap); setNewMap(''); setSelection(null); setQuery(''); return graph.id; }); }}>
        ...
        {snapshot && <>
          ...
          {selectedItem && selection && <ItemEditor key={`${snapshot.graph.id}:${selection.itemId}`} snapshot={snapshot} selection={selection} busy={busy} dirty={dirty} onDirty={setDirty} perform={perform} onRemoved={() => setSelection(null)} />}
        ...
        <p className="local-note">Saved in this Chrome profile. Export a backup to keep a separate copy. Google imports and PDF reading are coming next.</p>
      ...
      <section className="map-area" aria-label="Current map">
        ...
        {snapshot ? <GraphCanvas key={snapshot.graph.id} snapshot={snapshot} query={query} busy={busy || dirty}
...
function ItemEditor({ snapshot, selection, busy, dirty, onDirty, perform, onRemoved }: { snapshot: GraphSnapshot; selection: Selection; busy: boolean; dirty: boolean; onDirty: (dirty: boolean) => void; perform: (action: Action) => Promise<boolean>; onRemoved: () => void }) {
  ...
  const originalText = 'body' in item && !sourceNode ? item.body : override?.notes ?? '';
  ...
  return <section className="inspector"><h2>{selection.itemType === 'node' ? 'Edit node' : 'Edit connection'}</h2>
    <form onSubmit={(event) => void submit(event)}>
      ...
      {link && <a className="source-link" href={link} target="_blank" rel="noreferrer">Open destination ↗</a>}
