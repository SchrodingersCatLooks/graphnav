...
| Partner | M4-B / `partner-data` | M1-B and M2-B verified live (import, exact nested-tab navigation, persistence across a full Chrome restart). M4-B extraction now implemented: byte fingerprint, bookmark sections with preserved nesting, labeled heading fallback, and mapping into the repository. 12 focused tests cover the dispatcher and PDF paths against a labeled synthetic fixture. Node 22.23.2 type-check, production build, and all 33 tests pass. | Nothing merged: `main` is still at `240f9f2` while this branch carries M1-B, M2-B, M4-B extraction and Rajvansh's storage slice. PDF extraction is verified against a synthetic fixture only, never a real paper, and has no reader UI (M4-A) to run in. Rajvansh's own-account acceptance is still outstanding. | Merge this branch. Then verify extraction against a real text PDF once a reader entrypoint exists, and decide whether extraction runs in the worker or the reader page. |
