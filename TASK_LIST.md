...
| M4-B | Eddy | 9 | Supply PDF text, identity, destinations, and storage | REVIEW | Extractor and reader integration are merged through PR #10; durable bytes, exact reattachment and restart are already tested in PdfLibrary. Eddy’s additional 2e89d18 helper remains on partner-data and is not wired to the reader; PDF_HANDOFF records key-prefix, limit and eviction differences requiring reconciliation. Independent paper/multi-column and actual model acceptance remain open. |
...
| X2-A | Rajvansh | 11 | Add sources to an existing project map | DOING | After PR #14, expose the target map before source selection, reuse existing selected Doc/PDF imports, and preserve the chosen map when following a source into another surface. Verify a Doc-tab/PDF-section connection, independent maps, refresh and reopen. No proposal-store or Dexie schema edits; joint multi-source generation still needs X2-B. |
