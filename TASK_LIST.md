...
| G0-A | Rajvansh | 1 | Confirm API access, funding, and demo configuration | REVIEW | Replacement credential authenticated with GET /v1/models returning 200; configured gpt-5-mini is accessible. Secret remains only in ignored local configuration. Actual generated response, usage/funding proof and bounded demo setup are being checked; rotate the chat-exposed credential before final use. |
...
| --- | --- | --- | --- | --- |
| Rajvansh | G2 integration on `rajvansh-ui` | PR #12 client/UI is merged. Replacement credential authenticates and gpt-5-mini is accessible. Integrating the relay/server checks with Eddy main a8325dc. | Current main server duplicates the old input-hash algorithm, so a reply cannot match the client hash; Rajvansh has the shared-hash correction and provider request limits under test. | Finish the integrated relay fixture and one real selected-content generation; merge checked runtime. Eddy retains G3-B database/proposal/backup work. |
| Partner | G0-B / `partner-data` at `de16b0c` | Branch report: separate relay package/provider adapters, 89 of 90 tests pass; no model called. Work through 179d98f is already merged in PR #11. PDF bytes/reader already work on main; extra helper review is in PDF_HANDOFF. | Rajvansh’s replacement API credential now authenticates; real generation is in progress. G3-B file ownership is now explicitly assigned to Eddy, so approval for those files is no longer a blocker. | Read current GENERATION_HANDOFF before further shared-file work. Eddy owns G3-B migration/store/acceptance/backups; Rajvansh is integrating the relay client/server boundary. |
