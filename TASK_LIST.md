...
| Partner | M5-B on `partner-data`; next G0-B / G1-B | Eddy reports live Google import/navigation/restart. Runtime through 4738cb2 is now merged in PR #6; newer a5e7192 is documentation only. Combined 48 tests pass. | PDF has no reader or real-paper acceptance. API project/key ownership unresolved. Current main already includes selection/autofill and same-Doc navigation. | Bring main into partner-data normally, read EDITOR_HANDOFF and this tracker, review the shared bridge, then build the relay and selected-content extraction/draft contract. No force-push or reset. |
