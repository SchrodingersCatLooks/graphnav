...
| X4-A | Rajvansh | 13 | Optional movable floating panel | DOING | Claimed after PR #13 while Eddy implements G3 persistence. Add pointer/keyboard move/resize/reset/dock in the existing popover shell, with a separate versioned placement preference bridge. Preserve graph state and clamp controls after viewport changes. |
| X4-B | Eddy | 13 | Persist and restore panel placement | TODO | Rajvansh’s X4-A includes the minimal placement message and chrome.storage.local key; no Dexie or proposal files change. Eddy retains review of persistence/restart/display-change behavior at that checkpoint. |
