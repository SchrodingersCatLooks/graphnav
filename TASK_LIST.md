...
| UX2-A | Rajvansh | Now | Open saved maps, or choose manual/AI under New graph | DOING | Latest user correction: saved graph opens with Edit graph and New graph; no saved graph opens the creation chooser. Manually and With AI are sub-options of New graph, never sibling actions. On rajvansh-ui: stop automatic graph creation on empty sources, preserve saved maps, and reuse selected-content AI tools including supported Doc content from Drive. Reuse existing storage/generation APIs; G3 persistence and Eddy's backup work remain separate. |
