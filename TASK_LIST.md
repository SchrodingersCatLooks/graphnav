...
| Partner | M2-B / `partner-data` | M2-B complete and verified live. Import stored 11 Drive nodes and 5 Doc nodes; a repeat import reused the same `graphId`. `NAVIGATE` opened the exact nested Doc sub-tab and a real Drive folder from stored locators. Saved maps reopened intact after a full Chrome quit and relaunch. Dispatcher validates request shapes and senders, now covered by 5 focused tests. Node 22.23.2 type-check, production build, and all 27 tests pass. | Nothing merged: `main` is still at `240f9f2` while this branch carries M1-B, M2-B and Rajvansh's storage slice. Rajvansh's own-account acceptance is still outstanding, so the OAuth client and test-user list are unproven for any account but Eddy's. Existing two graphs keep the old `createdVia: 'manual'` value. | Merge this branch, then start M4-B PDF section extraction. M3-B is already covered by the Docs adapter and verified nested-tab navigation. |
