...
| UX4-A | Rajvansh | Now | Unified Add, adjustable editor/cards and optional connection labels | DOING | On rajvansh-ui from b73fa8d: implement the user's Add tree with lazy child loading, ranked name search and add-or-focus rows; enlarge/minimize/resize/fullscreen the editor; NodeResizer cards; forgiving click/drag connections; midpoint label pills and EdgeToolbar editing with direction controls. Extend existing overlay/layout records additively, preserve stable IDs and source refresh, and verify the complete label/resize/reopen/refresh loop. See UX4_HANDOFF for the shared storage seam; Eddy retains proposal decisions and backup integration. |
...
| --- | --- | --- | --- | --- |
| Rajvansh | UX4-A implementation on `rajvansh-ui` | [PR #18](https://github.com/SchrodingersCatLooks/graphnav/pull/18) (8aa97c0, runtime fa2c58c) merges the compact launcher and functional creation/saved-map/account steps. Both typechecks, build and all 139 tests (1.1 minutes) passed on pinned Node; screenshots inspected. | User feedback after extension reload and Google tab refresh; persistent AI proposal controls remain unwired. | Follow REVIEW_WALKTHROUGH, address concrete usability issues, then connect G3-A to PROPOSAL_STORE_HANDOFF without duplicating storage. |
