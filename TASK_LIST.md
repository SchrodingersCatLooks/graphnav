...
| M4-B | Eddy | 9 | Supply PDF text, identity, destinations, and storage | REVIEW | Extraction, SHA-256 identity, outline/heading locators, and now byte storage at `2e89d18` using the `blobs` table already in the version 1 schema, so no migration and none of Rajvansh's files changed. Demo paper round-trips byte-identical; re-opening reuses one copy; dropping bytes provably leaves map, source node, destination, personal idea and note intact; oversize/empty/quota/bad-key failures all surface with reasons. Not done: never run against a real paper, no multi-column inspection, no reader UI (M4-A). |
...
| Partner | G0-B / `partner-data` at `de16b0c` | Reported from branch, unmerged. Since PR #6: G1-B extraction and draft contract, demo paper, G2-B provider-agnostic request path, M6-B 500-node scale evidence (import 144ms, read 14ms, refresh 694ms) plus failure cases, PDF byte storage, and the local AI relay. 89 of 90 tests pass. Nothing touched in Rajvansh's files. | No model has ever been called; needs G0-A's key. PDF has no reader and no real-paper acceptance. G3-B proposal persistence deliberately not started: it needs a Dexie migration and a proposal store in Rajvansh's files. | Plug the key into the relay for a first real generation. G3-B needs an ownership decision before anyone starts it. |
