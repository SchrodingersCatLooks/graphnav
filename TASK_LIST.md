...
| G1-A | Rajvansh | 6 | Select and preview exactly what will be analyzed | DOING | Claimed in GENERATION_HANDOFF: integrate Eddy checkpoint, review input/request boundaries, add typed Docs preview and local PDF page extraction, then shared selection/preview UI.  Choose Doc tabs or, after M4, PDF pages; show selected content and limits; Generate requires a deliberate click; unsupported/oversize content is explained |
...
| --- | --- | --- | --- | --- |
| Rajvansh | G1-A on `rajvansh-ui` | PR #10 merged the manual PDF reader with 58 passing tests. Read latest main and partner-data at 179d98f. | API project owner/private credential unresolved. This does not block selected-content preview or mocked request/review work. | Integrate selected Doc/PDF preview and the generation contract according to GENERATION_HANDOFF; preserve Eddy relay and proposal-persistence ownership. |
| Partner | G2-B / `partner-data` at `25652a5` | Reported from branch, unmerged. Since the PR #6 merge: G1-B extraction and draft contract, the demo paper with three-level nesting, and the provider-agnostic request path. 68 of 69 tests pass. | No model called; a real provider adapter waits on G0-A, which Rajvansh is handling. PDF still has no reader and no real-paper acceptance. | Real provider adapter once a key exists; meanwhile M4-B real-paper checks and G3-B proposal persistence. |
