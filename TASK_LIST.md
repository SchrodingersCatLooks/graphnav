...
| G3-B | Eddy | 8 | Persist proposals, decisions, and accepted records safely | TODO | Ownership confirmed explicitly in GENERATION_HANDOFF: Eddy may edit the additive database migration, proposal store, graph/evidence types, repository acceptance/backup methods and storage tests. No extra file-owner approval is needed. Publish the typed store/message handoff before Rajvansh wires controls. Preserve old maps/backups, atomic acceptance and accepted/edited/rejected decisions across refresh/regeneration. |
...
| --- | --- | --- | --- | --- |
| Rajvansh | X4-A on `rajvansh-ui`; G3-A next after store handoff | PR #13 merged as 5fa1ba4. Typechecks/build/116 tests plus 14 affected final tests pass; one real PDF generation and evidence jump succeeded. | G3 controls depend on Eddy’s typed proposal store/decision API. | Implement floating move/resize/dock/reset and its small preference bridge without touching G3 storage files; keep checking current main for the store handoff. |
| Partner | G3-B on `partner-data` | G0/G2 server and real OpenAI path now merged/verified through PR #13. No API-authentication blocker remains for the shared demo. | G3 proposal persistence and typed UI handoff remain open; ownership is explicitly assigned in GENERATION_HANDOFF. | Bring main into partner-data, implement additive proposal/decision persistence and atomic acceptance/backups, and publish the typed API. Leave X4 shell/preference bridge to Rajvansh. |
