...
| G0-A | Rajvansh | 1 | Confirm API access, funding, and demo configuration | BLOCKED | User supplied project, organization and key metadata. A private ignored server configuration file is prepared on Rajvansh’s laptop; the first private GET /v1/models check returned 401 invalid_api_key after the user saved a value; replacement is pending. Live API access, funding and the test-spend cap remain unverified. Key IDs are not authentication credentials; no secret belongs in chat, GitHub or the extension. |
...
| G0-B | Eddy | 6 | Build the minimal local AI relay and startup path | REVIEW | Separate `relay/` package at `de16b0c`, no extension dependencies. Key lives only on the relay; the extension holds a shared token. Relay is graph-agnostic: the JSON schema arrives per request, generated by `z.toJSONSchema` from the same Zod object we validate against, so constraint and check cannot drift. Verified against the running server: health 200, wrong origin 403, wrong token 401, no key 503, unknown route 404, token absent from logs. Loopback only, constant-time token compare, 256KB cap enforced while reading, provider errors logged by name only. OpenAI and Gemini adapters both present so the provider decision stays open. Startup documented in relay/README.md. Untested against a real model. |
...
| G2-A | Rajvansh | 7 | Add real Generate and draft-preview states | REVIEW | PR #12 merged as c947fa1, runtime 148dfa4. Pairing/settings, bounded selected-content relay requests, progress/cancel/retry, source/account/map checks, validated draft display and evidence navigation pass typecheck/build/all 102 tests, including a real loopback HTTP fixture with installed Chromium. Actual model response remains open; integrate Eddy server de16b0c next. |
...
| --- | --- | --- | --- | --- |
| Rajvansh | G2 integration on `rajvansh-ui` | PR #12 merged extension relay/UI checkpoint; typecheck/build/all 102 tests pass with installed loopback fixture and inspected UI. | Private OpenAI credential was rejected with 401 invalid_api_key. Eddy server contract differs from the client and is not integrated yet. | Reconcile de16b0c relay with the merged client, preserving both implementations and focused checks. G3 database/proposal work is explicitly Eddy’s lane. |
| Partner | G0-B / `partner-data` at `de16b0c` | Branch report: separate relay package/provider adapters, 89 of 90 tests pass; no model called. Work through 179d98f is already merged in PR #11. PDF bytes/reader already work on main; extra helper review is in PDF_HANDOFF. | API authentication currently fails. G3-B file ownership is now explicitly assigned to Eddy, so approval for those files is no longer a blocker. | Bring main into partner-data and read GENERATION_HANDOFF. Eddy owns G3-B migration/store/acceptance/backups; Rajvansh is integrating the relay client/server boundary. |
