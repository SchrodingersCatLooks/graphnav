...
| Partner | M4-B / `partner-data` | M1-B and M2-B verified live. M4-B PDF extraction implemented and tested against a labeled synthetic fixture through real PDF.js. Fixed an expand gap: importing a subfolder created a second map instead of adding to the one on screen; imports now accept `intoGraphId`, and tests confirm the subfolder node is reused with its original ID rather than duplicated. Node 22.23.2 type-check, production build, and all 36 tests pass. | Nothing merged: `main` is still at `240f9f2`. Expand is verified at repository level only; no UI calls `intoGraphId` yet. PDF extraction has never run against a real paper and has no reader UI. `refreshScope` hardcodes `availability: 'available'`, so unavailable targets cannot be reported yet (M5-B). Rajvansh's own-account acceptance is still outstanding. | Merge this branch, then M5-B: unavailable-target reporting and verifying personal edits survive refresh. |
