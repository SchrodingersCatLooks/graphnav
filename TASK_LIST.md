...
| M2-B | 2–4h | Partner | Drive adapter, edit commands, navigation, initial save | REVIEW | Real sources plus manual nodes/edges save and reopen with stable IDs |
...
| M5-B | 8–10h | Partner | Preserve manual edits on refresh and reopening | DOING | Edits survive reload/refresh; missing targets handled; any implemented source write verified |
...
| Partner | M5-B / `partner-data` | M1-B and M2-B verified live. M4-B extraction implemented and tested. Expand now adds to the open map instead of creating a second one. M5-B: personal work is proven to survive a source refresh (renamed file keeps its node ID; idea, labeled connection, note and dragged position all survive), a file missing from a listing keeps its node and personal links, and unavailable targets can now be recorded via a direct check rather than inferred from absence. Node 22.23.2 type-check, production build, and all 40 tests pass. | Nothing merged: `main` is still at `240f9f2`. `CHECK_TARGETS` is not yet exercised against a real deleted file. No source write is implemented, and that needs a write scope plus re-consent on both accounts. PDF extraction has never run against a real paper and has no reader UI. Rajvansh's own-account acceptance is still outstanding. | Merge this branch, then M6-B: measured limits with a 1,000-node fixture, and decide whether a source write is in scope for the demo. |
